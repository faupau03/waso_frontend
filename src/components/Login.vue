<template>
    <div id="login" class="w-screen h-screen">
        ...
        <form action @submit.prevent="login" id="login-form" class="flex flex-col items-center mx-5">
            ...
            <div id="password-container" class="items-center flex relative w-full max-w-lg mb-5">
                ...
                <button type="button" class="absolute flex mr-3 right-0 top-0 text-center h-full items-center"
                    @click="togglePasswordVisible">
                    <b-icon-eye v-show="!passwordVisible" class="text-neutral-400 hover:text-white"></b-icon-eye>
                    <b-icon-eye-slash v-show="passwordVisible"
                        class="text-neutral-400 hover:text-white"></b-icon-eye-slash>
                </button>
            </div>
            <span v-show="error" class="text-red-500 text-sm mb-4">{{ error }}</span>
            <button v-if="loading" @click="login" disabled
            ...
            <button v-else @click="login" type="submit"
                class="w-full max-w-lg h-12 p-2 inline-flex items-center justify-center rounded-lg bg-violet-600 hover:bg-violet-900 hover:text-gray-400 text-white text-xl">Login</button>
